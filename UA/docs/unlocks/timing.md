...
`get_tick_count()` повертає кількість тіків (ticks), виконаних з початку виконання (execution).

Ці дві функції, а також `quick_print()`, є повністю безкоштовними (free). Навіть операція виклику для них є безкоштовною.
...
Базова одиниця часу для виконання коду називається "тік" (tick). Без покращень швидкості та енергії виконання відбувається зі швидкістю `400` тіків на секунду.
...
Виклики функцій (function calls) та читання і запис змінних є безкоштовними, але визначення функцій (function definitions) займають 1 тік.
...
Індексування структури даних займає один тік для оператора індексу, а у випадку словника (dictionary) або множини (set) — додаткові тіки залежно від розміру ключа.

Кількість тіків, яку займають вбудовані функції (builtin functions), задокументована в документації кожної функції окремо.
