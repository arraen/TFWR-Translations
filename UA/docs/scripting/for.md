...
Подібно до циклу `while`, цикл `for` також неодноразово викликає блок коду. Замість циклу на основі умови, він виконує тіло циклу один раз для кожного елемента в послідовності (sequence).
...
`variable_name` може бути будь-яким ім'ям, яке ви виберете. Це змінна, яка зберігає поточний елемент у послідовності. `sequence` має бути значенням, яке можна ітерувати, наприклад, діапазоном чисел (range). Блок коду виконується для кожного елемента, при цьому змінна циклу присвоюється цьому елементу.
